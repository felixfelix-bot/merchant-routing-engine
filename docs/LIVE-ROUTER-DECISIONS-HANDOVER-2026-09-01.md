...
- DeepInfra, PPQ, Telnyx, OpenRouter, routstr ×2 (per-token, two of them payable in sats)
...
1. **NO CAPS.** No provider is ever capped, blocked, or disabled for spending too much. Abnormal burn is *surfaced* as alerts, never blocked. If a provider is expensive or depleted, the market routes around it on the very next request — pressure re-prices, it does not prohibit.
...
Live proof it works: as of 2026-09-01 our primary z.ai key is weekly-locked (100% used, resets in ~43h). Nothing crashed, nothing was manually reconfigured — the flat router kept serving every request from the next-cheapest healthy providers, automatically, per request.
...
request(model, messages)
  ...
  3. health gate                    # backoff windows, paywall flags, circuit breaker,
  │                                 # .key_disabled_<name> markers, funding gate, live-catalog
  │                                 # phantom guard (skip candidates absent from a fresh
  │                                 # upstream catalog snapshot)
  4. price each survivor            # compute_effective_price() — 5-tier (§3)
...
|---|---|---|---|
| T1 | quota | z.ai ours + friend | sunk cost: `$0.001 × max(0.0001, days_to_reset/7)` — **price DECAYS toward the weekly reset** (see below) |
| T2 | balance | NeuralWatt | base rate × depletion penalty; 3.6× token-overcount correction (measured) |
...
`pressure(u) = 1 + K·t/(1-t)` where `t = (usage − onset)/(1 − onset)` — asymptotic: as usage → 100%, price → ∞ (router always finds an alternative first, by construction). Superposition: session × weekly × monthly factors MULTIPLY. Onsets are staggered per provider class (quota keys feel pressure at 60–70% used, credit-balance keys at 80%) so providers leave the market in a graceful order instead of all at once. A balance provider (NeuralWatt) is deliberately NOT given a depletion penalty in the same shape — the routstrd lesson (§7) showed depletion penalties on pay-per-token create a vicious circle when combined with failover; its two-phase model is priced by measured phase instead.

### The other multipliers (all deterministic — see §4 for why they are outside the Kalman)

- **peak_mult** — 3.0× for z.ai keys during peak hours (UTC 06:00–09:59), 1.0 otherwise. Deterministic by design ([ADR-003](https://github.com/felixfelix-bot/merchant-routing-engine/blob/main/docs/adr/ADR-003-deterministic-peak-multiplier.md)): peak scarcity is a schedule fact, not a measurement.
...
| `key_decisions` | routing decision (`reason` = ordered candidate chain, `chosen_key` = winner) | "why did traffic go there" forensics |
...
**Job 3 — incident forensics need the ordering chain.** When someone asks "why did we use paid X while cheap Y sat idle?", `key_decisions.reason` (the full ordered chain) vs `chosen_key` (the winner) answers it in one query. The answer is occasionally "Y appeared in every chain but never won — because it was health-excluded or dead", which is exactly the distinction a `used_pct` column cannot give you.
...
| **Urgency classification** (deferral) | *when may non-urgent work run?* | per task | NOW (money bleeding / hard deadline) dispatches anywhere; SOON waits for deadline; DEFER/BATCH wait for a cheap window (cheapest-eligible price ≤ p20 of trailing 7d hourly medians). Design: [`docs/DESIGN-urgency-enforcement.md`](https://github.com/felixfelix-bot/merchant-routing-engine/blob/main/docs/DESIGN-urgency-enforcement.md), [`docs/cost-gate.md`](https://github.com/felixfelix-bot/merchant-routing-engine/blob/main/docs/cost-gate.md). |
...
Full versions in [`docs/FRIEND-ONBOARDING-merchant-router-2026-09-01.md`](https://github.com/felixfelix-bot/merchant-routing-engine/blob/main/docs/FRIEND-ONBOARDING-merchant-router-2026-09-01.md) §6 and ADR lineage. The ones that shaped the decision logic in §3:
